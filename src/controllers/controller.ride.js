...
async function Finish(req, res){
    ...
    try{
        const ride_id = req.params.ride_id;      
        const passenger_user_id = req.body.passenger_user_id;
        console.log(ride_id, passenger_user_id);
...
async function ListForDriver(req, res){
    ...
    try{
        ...
        const status = req.query.status;      
        const rides =  await serviceRide.ListForDriver( driver_user_id, status);
        res.status(200).json(rides);
    }catch(error){
...
async function ListDetail(req, res) {
    try {
        ...
        if (rides.length > 0) {
            res.status(200).json(rides[0]); // Retorna apenas o primeiro item
        } else {
...
async function Accept(req, res) {
    ...
    try {
        ...
        console.log("Passsou aqui",ride_id);
